...
export default function PreviewPage() {
  const cart = useCart();
  useEffect(() => { cart.syncCatalog(kCatalog); }, []); // ensure itemsById
  ...
  const handleDownloadPdf = async () => {
    if (lines.length === 0) { alert("No items to download"); return; }
    const bytes = await generateCartPdfBytes({
      lines,
      totalItems: cart.totalItems,
      totalAmount: cart.totalAmount,
...
Attached: PDF summary.`;
    ...
    if ((navigator as any).canShare?.({ files: [file] })) {
      try {
        await (navigator as any).share({ title: "Chandini Hirers Order", text: shareText, files: [file] });
        return;
      } catch {}
    }
  ...
  const handleClearAll = () => {
    ...
    // Reset any existing preview URL and UI
    if (urlRef.current) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 pb-28">
      <header className="py-4 flex items-center justify-between">
        ...
        {/* Optional top-right Clear button as well */}
        <button
      ...
      {lines.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {!fontReady && (
            <div className="text-sm text-gray-600">
              Loading PDF fonts to display ₹ correctly…
            </div>
          )}
          {fontReady && !fontOk && (
          ...
          {/* Inline viewer (only after user asks and fonts OK) */}
          {showInline && fontOk && (
      ...
      <div className="fixed inset-x-0 bottom-0 bg-white border-t">
        <div className="max-w-5xl mx-auto px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button
            className="border rounded-xl py-3 disabled:opacity-50 text-gray-400"
          ...
          <button
            className="bg-green-600 text-white rounded-xl py-3 disabled:opacity-50"
            onClick={handleShare}
            disabled={lines.length === 0}
          >
            Order Now
