...
// Currency: ₹ with Indian grouping (fallback to "Rs " if fonts fail)
...
/* ============================================================
   Font loading — fetch once, register on every jsPDF doc
============================================================ */
...
/** Preload fonts early (e.g., on Preview mount) */
...
/** Ensure DejaVu is registered on THIS jsPDF doc */
async function ensureFonts(doc: jsPDF) {
  ...
  if (fontsOk && dejavuRegB64 && dejavuBoldB64) {
    ...
    doc.setFont("helvetica", "normal"); // fallback (₹ may not render)
  }
}
...
export async function generateCartPdfBytes(params: {
  ...
  doc.text("Amount", colAmtRight, y, { align: "right" } as any);
  ...
  const addPageIfNeeded = () => {
    if (y > pageHeight - margin - 60) {
      ...
      doc.text("Amount", colAmtRight, yy, { align: "right" } as any);
  ...
  for (const l of lines) {
    ...
    doc.text(amountStr, colAmtRight, y, { align: "right" } as any);
  ...
  doc.text(String(totalItems), colAmtRight, y, { align: "right" } as any);

  y += rowH;
  doc.text("Total Amount:", pageWidth - 250, y);
  doc.text(formatINR(totalAmount), colAmtRight, y, { align: "right" } as any);
...
export function robustDownloadPdf(bytes: ArrayBuffer, filename: string) {
  try {
    ...
    // iOS / Safari: opening in a new tab is more reliable
    const isIOS =
